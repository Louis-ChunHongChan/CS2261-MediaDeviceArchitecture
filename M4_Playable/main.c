/* Comment Block
The game has win/lose condition, the core gameplay mechanics (walk, jump, ladder climbing, power-ups pick up, cheat), time limit and visual indicator for time remaining, restart in pause menu and the caramel maps of the final version are finished.
Modify background tiles at runtime, sounds, XL background, art, and different enemies and corresponding animations still need to be added.
No bugs so far.
To play: Use left & right to move on a platform, up & down to move on a ladder, press B to jump over obstacles/enemies, collide with power-ups to pick them up. Try your best to reach the top of the level in the time limit to deliver the food to the customer who is waiting you there.
*/
...
void goToGame() {
    ...
    REG_DISPCTL = MODE3 | BG0_ENABLE | SPRITE_ENABLE;
    REG_BG0CNT = BG_CHARBLOCK(1) | BG_SCREENBLOCK(28) | BG_SIZE_TALL;
    DMANow(3, undergroundPal, PALETTE, undergroundPalLen / 2);
    DMANow(3, undergroundTiles, &CHARBLOCK[1], undergroundTilesLen / 2);
    DMANow(3, undergroundMap, &SCREENBLOCK[28], undergroundMapLen / 2);

    // FIXME
    REG_DISPCTL = MODE4 | BG2_ENABLE | SPRITE_ENABLE;
    // REG_BG2CNT = BG_CHARBLOCK(2) | BG_SCREENBLOCK(31) | BG_SIZE_TALL;
