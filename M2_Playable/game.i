...
void updateGame() {
    ...
    if (!climbing) {
        if (collisionCheck(collisionMap, 256, guy.col, guy.row, guy.width, guy.height, 0, gravity)) {
            guy.row += gravity;
        }
        if (gravity == 10) {
            if (collisionCheck(collisionMap, 256, guy.col, guy.row, guy.width, guy.height, 0, gravity/2)) {
                guy.row += gravity/2;
            }
        }
    }
    ...
    if (guy.row == 232 || collisionCheck(laddercollisionMap, 256, guy.col, guy.row, guy.width, guy.height, 0, -guy.rdel)) {
        climbing = 0;
    }
...
# 365 "game.c"
    ...
    if (!powerUp && !down && !jumping) {
        ...
        if((~((*(volatile unsigned short *)0x04000130)) & ((1 << 7)))) {
            if (!collisionCheck(laddercollisionMap, 256, guy.col, guy.row, guy.width, guy.height, 0, guy.rdel) && guy.row + guy.rdel <= 232) {
                guy.row += guy.rdel;
                climbing = 1;
