...
void initBubble() {
    bubble.width = 16;
    bubble.height = 16;
    bubble.col = SCREENWIDTH / 2 - bubble.width * 4;
    bubble.row = 60+256;
    bubble.active = 1;
}
...
void updateGame() {
    ...
    //     // need to change all the position of sprites
    //     // vOff = 96 +256;
    // }
...
void drawGuy() {
    shadowOAM[0].attr0 = ((guy.row - vOff) & ROWMASK) | ATTR0_SQUARE;
    shadowOAM[0].attr1 = ((guy.col - hOff) & COLMASK) | ATTR1_SMALL;
...
void drawDoor() {
    if (door.active && door.row - vOff > 0 && door.col - hOff > 0 && door.col - hOff < SCREENWIDTH && door.row - vOff < SCREENHEIGHT) {
        shadowOAM[15].attr0 = ((door.row - vOff) & ROWMASK) | ATTR0_TALL;
        shadowOAM[15].attr1 = ((door.col - hOff) & COLMASK) | ATTR1_MEDIUM;
...
void drawKey() {
    if (key.active && key.row - vOff > 0 && key.col - hOff > 0 && key.col - hOff < SCREENWIDTH && key.row - vOff < SCREENHEIGHT) {
        shadowOAM[16].attr0 = ((key.row - vOff) & ROWMASK) | ATTR0_SQUARE;
        shadowOAM[16].attr1 = ((key.col - hOff) & COLMASK) | ATTR1_SMALL;
...
void drawShirt() {
    if (shirt.active && shirt.row - vOff > 0 && shirt.col - hOff > 0 && shirt.col - hOff < SCREENWIDTH && shirt.row - vOff < SCREENHEIGHT) {
        shadowOAM[18].attr0 = ((shirt.row - vOff) & ROWMASK) | ATTR0_SQUARE;
        shadowOAM[18].attr1 = ((shirt.col - hOff) & COLMASK) | ATTR1_SMALL;
...
void drawBubble() {
    if (bubble.active && bubble.row - vOff > 0 && bubble.col - hOff > 0 && bubble.col - hOff < SCREENWIDTH && bubble.row - vOff < SCREENHEIGHT) {
        shadowOAM[19].attr0 = ((bubble.row - vOff) & ROWMASK) | ATTR0_SQUARE;
        shadowOAM[19].attr1 = ((bubble.col - hOff) & COLMASK) | ATTR1_SMALL;
...
void drawCrazy() {
    for (int i = 0; i < CRAZYCOUNT; i++) {
        shadowOAM[30+i].attr0 = ((crazy[i].row - vOff) & ROWMASK) | ATTR0_SQUARE;
        shadowOAM[30+i].attr1 = ((crazy[i].col - hOff) & COLMASK) | ATTR1_SMALL;
