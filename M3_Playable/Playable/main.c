/* Comment Block
The game has win/lose condition, the core gameplay mechanics (walk, jump, ladder climbing, power-ups pick up, cheat), time limit and visual indicator for time remaining, and the caramel maps of the final version are finished.
Modify background tiles at runtime, sounds, art, and different enemies and corresponding animations still need to be added.
No bugs so far.
...
void goToGame() {
    ...
    // XL BG
    // DMANow(3, groundPal, PALETTE, 256);
    // DMANow(3, groundTiles, &CHARBLOCK[1], groundTilesLen / 2);
    // DMANow(3, groundMap, &SCREENBLOCK[29], groundMapLen / 2);
    state = GAME;
}
