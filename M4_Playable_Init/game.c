...
void initBubble() {
    for (int i = 0; i < 2; i++) {
        if (i == 0) {
            bubble[i].width = 16;
            bubble[i].height = 16;
            bubble[i].col = SCREENWIDTH / 2 - bubble[i].width * 4 - 10;
            bubble[i].row = 60+256;
            bubble[i].active = 1;
        }
        else {
...
void updateGuy() {
    ...
    for (int i = 0; i < CRAZYCOUNT; i++) {
        ...
        // guy destroys barrels with hammers
        // else {
    ...
    // if (vOff < 512) {
    //     vOff += 256;
    //     REG_BG0CNT = BG_CHARBLOCK(1) | BG_SCREENBLOCK(29) | BG_SIZE_TALL | BG_4BPP;
    //     // FIXME: still needs to reposition sprites
    // }
    // if (vOff < 256) {
    //     vOff += 256;
