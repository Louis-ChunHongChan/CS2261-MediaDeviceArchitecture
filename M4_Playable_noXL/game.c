...
void updateGame() {
    ...
    if (frame % 10 == 0) {
        if (upward == 20) {
            ...
            SCREENBLOCK[30].tilemap[OFFSET(13, 21, 32)] = SCREENBLOCK[30].tilemap[OFFSET(1, 21, 32)];
            SCREENBLOCK[30].tilemap[OFFSET(14, 21, 32)] = SCREENBLOCK[30].tilemap[OFFSET(1, 21, 32)];
            upward = 29;
        }
        else {
            ...
            SCREENBLOCK[30].tilemap[OFFSET(13, upward+1, 32)] = SCREENBLOCK[30].tilemap[OFFSET(1, 21, 32)];
            SCREENBLOCK[30].tilemap[OFFSET(14, upward+1, 32)] = SCREENBLOCK[30].tilemap[OFFSET(1, 21, 32)];
            upward--;
        }
    }

    // falling
    if (guyvOff+1 < (MAPHEIGHT - SCREENHEIGHT) && (guy.row - guyvOff) > SCREENHEIGHT / 2) {
        guyvOff++;
        vOff++;
    }
    if (guyvOff-1 > 0 && guy.row - guyvOff <= SCREENHEIGHT / 2) {
    ...
    if (!powerUp && !down && !jumping && !floating) {
        ...
        if(BUTTON_HELD(BUTTON_DOWN)) {
            if (collisionLadderCheck(collisionMap, MAPWIDTH, guy.col + guy.width/2 - 1, guy.row, guy.width/4, guy.height, 0, guy.rdel)) {
                ...
                // vOff+1 < MAPHEIGHT - SCREENHEIGHT && (guy.row - guyvOff) > (SCREENHEIGHT / 2)
    ...
    if (!climbing && !jumping && !down) {
        if(BUTTON_HELD(BUTTON_LEFT)) {
            if (collisionCheck(collisionMap, MAPWIDTH, guy.col, guy.row, guy.width, guy.height, -guy.cdel, 0) && guy.col > 0) {
                ...
                // hOff-1 >= 0 && (guy.col - hOff) <= (SCREENWIDTH / 2)
...
void updateCrazy() {
    ...
    for (int i = 0; i < CRAZYCOUNT; i++) {
        ...
        if (i == 6) {
            ...
            if (crazy[i].col <= SCREENWIDTH / 2 - crazy[i].height * 2) {
                crazy[i].cdel = 1;
            }
            crazy[i].col += crazy[i].cdel;
        }
...
void animateGuy() {
    ...
    if (!climbing && !down && !jumping) {
            if(BUTTON_HELD(BUTTON_LEFT) && !powerUp && !floating) {
                guy.aniState = LEFT;
            }
            if(BUTTON_HELD(BUTTON_RIGHT) && !powerUp && !floating) {
                guy.aniState = RIGHT;
            }
            if (!powerUp && !down && !jumping && !floating) {
                // int h = 25;
