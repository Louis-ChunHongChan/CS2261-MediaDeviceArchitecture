...
void initCrazy() {
    ...
    for(int i = 0; i < 2; i++){
        ...
        if (i == 1) {
            crazy[i].row = 68;
            crazy[i].col = 240 / 2 + 52;
        }
        else {
            crazy[i].row = 79;
            crazy[i].col = 240 / 2 - 80;
        }
...
void updateGame() {
    ...
    if (vOff+1 < 256 - 160 && (guy.row - vOff) > (160 / 2)) {
        vOff++;
    }
...
# 361 "game.c"
    if (!climbing && !jumping && !floating) {
        int kept = gravity;
        while (!collisionCheck(collisionMap, 256, guy.col, guy.row, guy.width, guy.height, 0, kept) && kept > 0) {
            kept--;
        }
        guy.row += kept;
    }
    ...
    if (collisionCheck(laddercollisionMap, 256, guy.col, guy.row, guy.width, guy.height, 0, -guy.rdel)) {
        climbing = 0;
    }
...
# 611 "game.c"
    for (int i = 0; i < 2; i++) {
        if (!crazy[i].active) {
            ...
            if (crazy[i].active) {
                if (i == 0) {
                    if (collisionCheck(collisionMap, 256, crazy[i].col, crazy[i].row, crazy[i].width, crazy[i].height, crazy[i].cdel, 0)) {

                        if (crazy[i].col >= 240 / 2 - 70) {
                            crazy[i].cdel = -1;

                        }
                        if (crazy[i].col <= 240 / 2 - 95) {
                            crazy[i].cdel = 1;
                        }
                    ...
                    if (collisionCheck(collisionMap, 256, crazy[i].col, crazy[i].row, crazy[i].width, crazy[i].height, crazy[i].cdel, 0)) {
                        if (crazy[i].col >= 240 / 2 + 110) {
                            crazy[i].cdel = -1;

                        }
                        if (crazy[i].col <= 240 / 2 + 35) {
                            crazy[i].cdel = 1;
                        }
                        crazy[i].col += crazy[i].cdel;
                    }
...
void animateGuy() {
    ...
    if (!climbing && !down && !jumping) {
            ...
            if (!powerUp && !down && !jumping && !floating) {
                if (guy.aniState == LEFT && (!(~(oldButtons) & ((1 << 1))) && (~buttons & ((1 << 1))))) {
                    ...
                    if (collisionCheck(collisionMap, 256, guy.col, guy.row, guy.width, guy.height, -15, -15)) {
                        if (jumpBoosted) {
                            guy.row -= 70;
                            guy.col -= 25;
                        }
                ...
                else if (guy.aniState == RIGHT && (!(~(oldButtons) & ((1 << 1))) && (~buttons & ((1 << 1))))) {
...
void drawCrazy() {
    for (int i = 0; i < 2; i++) {
        shadowOAM[3+i].attr0 = (crazy[i].row - vOff) | (0 << 14);
        shadowOAM[3+i].attr1 = (crazy[i].col - hOff) | (1 << 14);
        ...
        shadowOAM[3+i].attr2 = ((0)*32 + (17)) | ((0) << 12);
    }
}
