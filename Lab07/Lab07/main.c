...
int main() {
    ...
    while(1) {
        ...
        if(link.aniCounter % 15 == 0) {
            ...
            link.curFrame++;
            link.curFrame = link.curFrame % link.numFrames;
        ...
        shadowOAM[0].attr2 = ATTR2_TILEID(link.aniState * 4, link.curFrame * 4) | ATTR2_PALROW(0);
        ...
        for (int i = 0; i < FAIRYCOUNT; i++) {
            ...
            if (fairy[i].aniCounter % 12 == 0) {
                fairy[i].curFrame++;
                fairy[i].curFrame = fairy[i].curFrame % fairy[i].numFrames;
            }
        }
        ...
        for (int i = 0; i < FAIRYCOUNT; i++) {
            shadowOAM[i+1].attr0 = fairy[i].row | ATTR0_4BPP | ATTR0_SQUARE;
            shadowOAM[i+1].attr1 = fairy[i].col | ATTR1_SMALL;
            shadowOAM[i+1].attr2 = ATTR2_TILEID(fairy[i].aniState, fairy[i].curFrame * 2 + 12) | ATTR2_PALROW(0);
        }
