...
# 112 "main.c"
        ...
        shadowOAM[0].attr2 = ((link.curFrame * 4)*32+(link.aniState * 4)) | ((0)<<12);
# 128 "main.c"
        for (int i = 0; i < 4; i++) {
            fairy[i].aniCounter++;
...
# 147 "main.c"
        for (int i = 0; i < 4; i++) {
            shadowOAM[i+1].attr0 = fairy[i].row | (0<<13) | (0<<14);
            shadowOAM[i+1].attr1 = fairy[i].col | (1<<14);
            shadowOAM[i+1].attr2 = ((fairy[i].curFrame * 2 + 12)*32+(fairy[i].aniState)) | ((0)<<12);
        }
