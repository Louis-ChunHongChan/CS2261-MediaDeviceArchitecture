...
void setPixel4(int col, int row, unsigned char colorIndex) {
    ...
    u16 offset = OFFSET(col, row, 240) / 2;
    u16 oldPixels = videoBuffer[offset];
    // if the px is odd px
    if (col & 1) {
        videoBuffer[offset] = (colorIndex << 8) | (oldPixels & 0x00FF);
    }
    else {
        videoBuffer[offset] = (oldPixels & 0xFF00) | colorIndex;
    }
...
void drawRect4(int col, int row, int width, int height, volatile unsigned char colorIndex) {
    ...
    if (width == 1) {
        for(int r = 0; r < height; r++) {
            setPixel4(col, row, colorIndex);
        }
    }
    else if (width == 2) {
        for(int r = 0; r < height; r++) {
            setPixel4(col, row, colorIndex);
            setPixel4(col + 1, row, colorIndex);
        }
    }
    // col and width are odd
    else if ((col & 1) && (width & 1)) {
        if (width-1 > 0) {
            for(int r = 0; r < height; r++) {
                setPixel4(col, row + r, colorIndex);
                DMANow(3, &color, &videoBuffer[OFFSET(col + 1, row + r, SCREENWIDTH)/2], DMA_SOURCE_FIXED | ((width-1)/2));
	        }
        }
    }
    // col is odd and width is even
    else if (col & 1) {
        if (width-2 > 0) {
            for(int r = 0; r < height; r++) {
                setPixel4(col, row + r, colorIndex);
                DMANow(3, &color, &videoBuffer[OFFSET(col + 1, row + r, SCREENWIDTH)/2], DMA_SOURCE_FIXED | ((width-2)/2));
                setPixel4(col + width - 1, row + r, colorIndex);
            }
        }
        
    }
    // width is odd and col is even
    else if (width & 1) {
        if (width-1 > 0) {
            for(int r = 0; r < height; r++) {
                DMANow(3, &color, &videoBuffer[OFFSET(col, row + r, SCREENWIDTH)/2], DMA_SOURCE_FIXED | ((width-1)/2));
                setPixel4(col + width - 1, row + r, colorIndex);
            }
        }
        
    }
    // col and width both even
    else {
        for(int r = 0; r < height; r++) {
